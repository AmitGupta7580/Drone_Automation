...
ref1 = get_location_metres(vehicle1.location.global_frame, 0, aSize, 10)   # 1 circle of vehicle 1
ref2 = get_location_metres(vehicle1.location.global_frame, aSize, 0, 10)   # 1 circle of vehicle 2
ref3 = get_location_metres(vehicle1.location.global_frame, 0, -aSize, 10)  # 2 circle of vehicle 1
ref4 = get_location_metres(vehicle1.location.global_frame, -aSize, 0, 10)  # 2 circle of vehicle 2
point1c_1=[]
point2c_1=[]
point1c_2=[]
point2c_2=[]
omega = math.pi/25
for i in range(1,50) :
    point1c_1.append(get_location_metres(ref1, aSize * (math.sin(omega * i)), -aSize * (math.cos(omega * i)), 12))
for i in range(1,50) :
    point1c_2.append(get_location_metres(ref2, -aSize * (math.cos(omega * i)), aSize * (math.sin(omega * i)), 9))
for i in range(1,50) :
    point2c_1.append(get_location_metres(ref3, aSize * (math.sin(omega * i)), aSize * (math.cos(omega * i)), 12))
for i in range(1,50) :
    point2c_2.append(get_location_metres(ref4, aSize * (math.cos(omega * i)), aSize * (math.sin(omega * i)), 9))
...
arm_and_takeoff(12, vehicle1)
arm_and_takeoff(9, vehicle2)
...
i = 0
while(i<len(point1c_1)) :
    vehicle1.simple_goto(point1c_1[i],groundspeed=40)
    vehicle2.simple_goto(point1c_2[i],groundspeed=40)
    i += 1
    time.sleep(3)

i = 0
while(i<len(point1c_1)) :
    vehicle1.simple_goto(point2c_1[i],groundspeed=40)
    vehicle2.simple_goto(point2c_2[i],groundspeed=40)
    i += 1
    time.sleep(3)
